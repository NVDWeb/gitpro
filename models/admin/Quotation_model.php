...
class Quotation_model extends CI_Model {
	...
	public function getQuotation($businessId,$individualId){
		...
		$select = '';
		if($businessId){
			$select = 'tbl_business.first_name';
			...
			$select = 'tbl_business.first_name,tbl_individual.name';
		...
		if($query->num_rows()>0){
			...
			foreach($result = $query->result() as $row){
				...
				$this->db->where('verifiedbyadmin',1);
	...
	public function getQuotationById($id){
		...
		if($id){
			...
			if($this->session->userdata('admin_user_type')==2){
				...
				$this->db->where('tbl_quotation.assignedbusiness_id',$this->session->userdata('adminId'));
			}elseif($this->session->userdata('admin_user_type')==3){
				$select = 'tbl_individual.name';
				$this->db->join('tbl_individual','tbl_individual.id=tbl_quotation.created_by','LEFT');
				$this->db->where('tbl_quotation.created_by',$this->session->userdata('adminId'));

			}
			$this->db->select('tbl_quotation.*,'.$select.' ');
			$this->db->from('tbl_quotation');
	...
	public function getLeadById($id){
		...
		if($id){
			...
			$this->db->join('tbl_team_request','tbl_team_request.team_category=tbl_quotation.team_category');
			if($this->session->userdata('admin_user_type')==1){
	...
	public function getRecommendation($category,$sub_category){
		...
		if($category && $sub_category){
			$this->db->select('tbl_business_category_relation.*,tbl_business.id as bId,tbl_business.first_name as businessName,tbl_business.email,tbl_business.email_notification,tbl_ratings.rating');
	...
	public function getTeamRecommendation($category){
		$result = array();
		if($category){
			$this->db->select('tbl_team_request.*,tbl_business.id as bId,tbl_business.first_name as businessName,tbl_business.email,MAX(tbl_team_ratings.rating) as maxRating');
			$this->db->from('tbl_team_request');
			$this->db->join('tbl_business','tbl_business.id=tbl_team_request.created_by','LEFT');
			$this->db->join('tbl_team_ratings','tbl_team_ratings.team_id=tbl_team_request.id','LEFT');
			$this->db->where('tbl_team_request.team_category',$category);
			$this->db->where('tbl_team_ratings.rating',5);
			$this->db->limit(5);
			$this->db->group_by('tbl_team_request.id');
			...
			// 	if($row->rating!=''){
			// 		$this->db->select('tbl_team_ratings.rating as MaxRating');
			// 		$this->db->from('tbl_team_ratings');
			// 		$this->db->where('tbl_team_ratings.team_id',$row->id);
			// 		$query = $this->db->get();
			// 		$res = $query->result();
			// 		$getTeamRecommendation['MaxRating'][$row->id] = $res[0]->MaxRating;
			// 	}
	...
	public function getAllProposalsByQid($q_id){
		$result = array();
		if($q_id){
			$this->db->select('tbl_proposal.*,tbl_quotation.id as quotationId,tbl_quotation.project_name,tbl_quotation.project_status,tbl_quotation.assignedbusiness_id,tbl_business.id as businessId,tbl_business.first_name');
	  ...
	  if($this->session->userdata('admin_user_type') !=1){
    	$this->db->where('tbl_quotation.created_by',$this->session->userdata('adminId'));
    	$this->db->where('tbl_proposal.verifiedbyadmin',1);
  		}
	...
	public function getProposalById($p_id){
		$result = array();
		if($p_id){
			$this->db->select('tbl_proposal.*,tbl_quotation.team_category,tbl_quotation.assignedbusiness_id,tbl_quotation.assignTeam,tbl_quotation.id as quotationId,tbl_quotation.project_name,tbl_quotation.bidclosingamount,tbl_business.id as businessId,tbl_business.first_name,tbl_business.email,tbl_team_request.team_name,tbl_team_request.team_category,tbl_team_request.created_by,tbl_team_request.id as teamId');
  			$this->db->from('tbl_proposal');
  			$this->db->join('tbl_quotation','tbl_quotation.id=tbl_proposal.q_id', 'LEFT');
      		$this->db->join('tbl_business','tbl_business.id=tbl_proposal.b_id', 'LEFT');
			...
			if(!empty($result)){
				...
				$this->db->where('business_id',$result[0]->b_id);
				$query = $this->db->get('tbl_bid');
				$get = $query->result();
				$result['bid']=$get;
			}
	...
	public function insertCronForProfessional($postData){
		//echo '<pre>';print_r($postData);
		$result=array();
		if($postData){
			$result = $this->db->insert('cron_table_professional',$postData);
		}
		return $result;
	}
	...
	public function getInterviewScheduledByQuotationId($q_id,$b_id){
		...
		if($q_id){
  		...
  		$this->db->where('tbl_interview.b_id',$b_id);
  ...
  public function getAssignedProject($businessId){
	...
	public function getTeamLeads($businessId,$iii){
		...
		/*getAll category and subcategory of business and then select leads based on these param*/
			...
			if($res){
				...
				$this->db->where('tbl_quotation.is_verifed',1);
				$this->db->from('tbl_quotation');
				$query = $this->db->get();
			}
	...
	public function getAdminListQuotation(){
			$result = array();
			$select = 'tbl_business.business_name as assignedTo,tbl_individual.name,tbl_quotation.category,tbl_quotation.sub_category';
			$this->db->join('tbl_business','tbl_business.id=tbl_quotation.assignedbusiness_id','LEFT');
			$this->db->join('tbl_individual','tbl_individual.id=tbl_quotation.individual_id','LEFT');
			$this->db->select('tbl_quotation.*,'.$select.',tbl_category.category_name');
			$this->db->join('tbl_category','tbl_category.id=tbl_quotation.category','LEFT');
		...
		if($query->num_rows()>0){
			foreach($result = $query->result() as $row){
				...
				$this->db->select('tbl_category.category_name');
				$this->db->where('parent_id',$row->category);
				$this->db->where('id',$row->sub_category);
				$query = $this->db->get('tbl_category');
				$getc = $query->result();				
	...
	public function getProposal($uId,$qId){
			$result = array();
			$this->db->select('tbl_proposal.b_id as proposalBusinessId,tbl_proposal.proposal_accepted,tbl_proposal.verifiedbyadmin,tbl_proposal.proposaltime,tbl_proposal.proposaltimelimit,tbl_proposal.proposalscopeofwork');
			$this->db->from('tbl_proposal');
			$this->db->where('tbl_proposal.q_id',$qId);
			$this->db->where('tbl_proposal.b_id',$uId);
			$query = $this->db->get();
			$result = $query->result();
			return $result;
	}
	...
	public function getInterview($uId,$qId){
		...
		$this->db->select('tbl_interview.id as interviewId,tbl_interview.schedule_date_time,tbl_interview.b_id as interviewBusinessId,tbl_interview.interview_accepted');
		$this->db->from('tbl_interview');
		$this->db->where('tbl_interview.q_id',$qId);
		$this->db->where('tbl_interview.b_id',$uId);
		$query = $this->db->get();
		$result = $query->result();
		return $result;
	}
	...
	public function getCompletedProject($businessId){
		...
		$this->db->join('tbl_category','tbl_category.id=tbl_quotation.category','LEFT');
		$this->db->where('tbl_quotation.assignedbusiness_id',$businessId);
		$this->db->where('tbl_quotation.project_status',1);		
		$this->db->from('tbl_quotation');			
		$query = $this->db->get();		
		if($query->num_rows()>0){
			foreach($result = $query->result() as $row){
 ...
 public function getAllCompletedProject(){
  $result = array();  
  $this->db->select('tbl_quotation.*,tbl_business.id as businessId,tbl_business.first_name');
    $this->db->from('tbl_quotation');    
       $this->db->join('tbl_business','tbl_business.id=tbl_quotation.assignedbusiness_id', 'LEFT');
    $this->db->where('tbl_quotation.created_by',$this->session->userdata('adminId'));
    $this->db->where('tbl_quotation.project_status',1);
 ...
 $result=array();
  if($postData){
   $this->db->set('created_date', 'NOW()', FALSE);
   $result = $this->db->insert('tbl_team_ratings',$postData);
  }
  return $result;
 }
 ...
 public function updateTeamRatings($postData,$team_id,$ind_id){
  ...
  if($postData){
   ...
   $result = $this->db->update('tbl_team_ratings',$postData);
  }
  return $result;
 }
 ...
 public function getTeamRatingsByClient($team_id,$ind_id){
  $result = array();
  if($team_id && $ind_id){
   $this->db->select('tbl_team_ratings.*');
   $this->db->where('tbl_team_ratings.team_id',$team_id);
   $this->db->where('tbl_team_ratings.ind_id',$ind_id);
   $query = $this->db->get('tbl_team_ratings');
   $result = $query->result();
  }
  return $result;
 }
 ...
 public function getTeamRatings($team_id){
  $result = array();
  if($team_id){
   $this->db->select('COUNT(tbl_team_ratings.team_id) as totalTeamReviews , max(tbl_team_ratings.rating) as teamrating');
   $this->db->from('tbl_team_ratings');
   $this->db->where('tbl_team_ratings.team_id',$team_id);
   $query = $this->db->get();
   $result = $query->result();

   $this->db->select('tbl_team_ratings.ind_id,tbl_team_ratings.admin_id');
   $this->db->where('tbl_team_ratings.team_id',$team_id);
   $query = $this->db->get('tbl_team_ratings');
   $get = $query->result();
   $result[]=$get;
 ...
 public function getTeamRatingsByAdmin($team_id,$ind_id){
  ...
  if($team_id && $ind_id){
   $this->db->select('tbl_team_ratings.*');
   $this->db->where('tbl_team_ratings.team_id',$team_id);
   $this->db->where('tbl_team_ratings.admin_id',$ind_id);
   $query = $this->db->get('tbl_team_ratings');
   $result = $query->result();
  }
  //echo '<pre>';print_r($result);
 ...
 public function totalTeamReviews($team_id){
  $result = array();
  if($team_id){
   $this->db->select('tbl_team_ratings.*,tbl_individual.name');
   $this->db->from('tbl_team_ratings');
   $this->db->join('tbl_individual','tbl_individual.id=tbl_team_ratings.ind_id','LEFT');
   $this->db->where('tbl_team_ratings.team_id',$team_id);
   $query = $this->db->get();
   $result = $query->result();
  }
  ...
  return $result;
	...
	public function insertQuotationRecomendation($postData){
		$result=array();
		if($postData){
			$this->db->set('send_date', 'NOW()', FALSE);
			$result = $this->db->insert('tbl_cron_quotation_recomendation',$postData);
		}
		return $result;
	}
 
	public function getQuoReco($q_id,$b_id){
		$result = array();
		if($q_id && $q_id){
			$this->db->select('tbl_cron_quotation_recomendation.*');
			$this->db->where('tbl_cron_quotation_recomendation.quotation_id',$q_id);
			$this->db->where('tbl_cron_quotation_recomendation.member_id',$b_id);
			$query = $this->db->get('tbl_cron_quotation_recomendation');
			$result = $query->result();
		}
		//echo '<pre>';print_r($result);
		return $result;
	...
	public function getProfessionalByCategorySubcatTypeofprojectAndPreferedmode($category=0,$sub_category=0,$type_of_project=0,$prefered_mode=0){
			$result= array();			
			$query = "select * from tbl_quotation where project_status=0";			
	        if($category && $sub_category=='' && $type_of_project=='' && $prefered_mode=='' ){
				if(sizeof($category) > 0){
	            	$ipchk_query =   implode(",",$category);
	            	$query .= " AND (category IN ($ipchk_query))";
	        	}
			}
			if($category && $sub_category && $type_of_project=='' && $prefered_mode=='' ){
					$ipchk_queryc1 =   implode(",",$category);
					$ipchk_queryc2 =   implode(",",$sub_category);
	            	$query .= " AND (sub_category IN ($ipchk_queryc2) AND category IN($ipchk_queryc1) )";
			...
			if($category && $sub_category && $type_of_project && $prefered_mode=='' ){
					$ipchk_queryc1 =   implode(",",$category);
					$ipchk_queryc2 =   implode(",",$sub_category);
					//$ipchk_queryc3 =   implode(",",$type_of_project);
	            	//$query .= " AND (sub_category IN ($ipchk_queryc2) AND category IN($ipchk_queryc1) AND type_of_project IN('$ipchk_queryc3') )";
					if($category && $sub_category && $type_of_project[0] && empty($type_of_project[1])){
						$query .= " AND (category IN ($ipchk_queryc1) AND sub_category IN ($ipchk_queryc2) AND (type_of_project ='$type_of_project[0]'))";
					}elseif($category && $sub_category && $type_of_project[0] && $type_of_project[1]){
						$query .= " AND (category IN ($ipchk_queryc1) AND sub_category IN ($ipchk_queryc2) AND (type_of_project ='$type_of_project[0]' OR type_of_project ='$type_of_project[1]'))";
					}
			...
			if($category && $sub_category=='' && $type_of_project && $prefered_mode=='' ){
					$ipchk_queryc1 =   implode(",",$category);				
					//$ipchk_queryc3 =   implode(",",$type_of_project);
	            	//$query .= " AND category IN($ipchk_queryc1) AND type_of_project IN('$ipchk_queryc3') )";
					if($category && $type_of_project[0] && empty($type_of_project[1])){
						$query .= " AND (category IN ($ipchk_queryc1) AND (type_of_project ='$type_of_project[0]'))";
					}elseif($category && $type_of_project[0] && $type_of_project[1]){
						$query .= " AND (category IN ($ipchk_queryc1) AND (type_of_project ='$type_of_project[0]' OR type_of_project ='$type_of_project[1]'))";
					}
			...
			if($category && $sub_category=='' && $type_of_project && $prefered_mode){
					$ipchk_queryc1 =   implode(",",$category);				
					$ipchk_queryc3 =   implode(",",$type_of_project);
					$ipchk_queryc4 =   implode(",",$prefered_mode);
	            	$query .= " AND category IN($ipchk_queryc1) AND type_of_project IN('$ipchk_queryc3') AND prefered_mode IN('$ipchk_queryc4'))";
			...
			if($category && $sub_category=='' && $type_of_project=='' && $prefered_mode){
					$ipchk_queryc1 =   implode(",",$category);				
					$ipchk_queryc4 =   implode(",",$prefered_mode);
	            	$query .= " AND category IN($ipchk_queryc1) AND prefered_mode IN('$ipchk_queryc4'))";
			...
			if($category && $sub_category && $type_of_project =='' && $prefered_mode){
					...
					$ipchk_queryc2 =   implode(",",$sub_category);				
					$ipchk_queryc4 =   implode(",",$prefered_mode);
	            	$query .= " AND category IN($ipchk_queryc1) AND sub_category IN ($ipchk_queryc2) AND prefered_mode IN('$ipchk_queryc4'))";
			...
			if($category && $sub_category && $type_of_project && $prefered_mode ){
					...
					$ipchk_queryc3 =   implode(",",$type_of_project);
					$ipchk_queryc4 =   implode(",",$prefered_mode);
	            	$query .= " AND (sub_category IN ($ipchk_queryc2) AND category IN($ipchk_queryc1) AND type_of_project IN('$ipchk_queryc3') AND prefered_mode IN('$ipchk_queryc4') )";
			...
			if($category =='' && $sub_category && $type_of_project && $prefered_mode ){					
					...
					$ipchk_queryc3 =   implode(",",$type_of_project);
					$ipchk_queryc4 =   implode(",",$prefered_mode);
	            	$query .= " AND (sub_category IN ($ipchk_queryc2) AND type_of_project IN('$ipchk_queryc3') AND prefered_mode IN('$ipchk_queryc4') )";
					
			}
			if($category =='' && $sub_category=='' && $type_of_project && $prefered_mode ){						
					$ipchk_queryc3 =   implode(",",$type_of_project);
					$ipchk_queryc4 =   implode(",",$prefered_mode);
	            	$query .= " AND ( type_of_project IN('$ipchk_queryc3') AND prefered_mode IN('$ipchk_queryc4') )";
					
			}
			
			if($category =='' && $sub_category=='' && $type_of_project =='' && $prefered_mode ){					
					$ipchk_queryc4 =   implode(",",$prefered_mode);
	            	$query .= " AND ( prefered_mode IN('$ipchk_queryc4') )";
			...
			if($category =='' && $sub_category && $type_of_project =='' && $prefered_mode ){
					$ipchk_queryc2 =   implode(",",$sub_category);					
					$ipchk_queryc4 =   implode(",",$prefered_mode);
	            	$query .= " AND ( sub_category IN ($ipchk_queryc2) AND prefered_mode IN('$ipchk_queryc4') )";
			...
			if($category =='' && $sub_category && $type_of_project && $prefered_mode=='' ){
					$ipchk_queryc2 =   implode(",",$sub_category);
					//$ipchk_queryc3 =   implode(",",$type_of_project);					
	            	//$query .= " AND ( sub_category IN ($ipchk_queryc2) AND type_of_project IN('$ipchk_queryc3'))";
					if($sub_category && $type_of_project[0] && empty($type_of_project[1])){
						$query .= " AND (sub_category IN ($ipchk_queryc2) AND (type_of_project ='$type_of_project[0]'))";
					}elseif($sub_category && $type_of_project[0] && $type_of_project[1]){
						$query .= " AND (sub_category IN ($ipchk_queryc2) AND (type_of_project ='$type_of_project[0]' OR type_of_project ='$type_of_project[1]'))";
					}
					
			}
			
			if($category =='' && $sub_category=='' && $type_of_project && $prefered_mode=='' ){
				//echo '<pre>';print_r($type_of_project);	
				//$data['type_of_project'] = $type_of_project;	
				//echo '<pre>';print_r($data);			
				//$ipchk_queryc3 =   implode(",",$type_of_project);
				if($type_of_project[0] && empty($type_of_project[1])){
					$query .= " AND (type_of_project ='$type_of_project[0]')";
				}elseif($type_of_project[0] && $type_of_project[1]){
					$query .= " AND (type_of_project ='$type_of_project[0]' OR type_of_project ='$type_of_project[1]')";
				}		
					
			}
			if($category =='' && $sub_category && $type_of_project=='' && $prefered_mode=='' ){					
					$ipchk_queryc2 =   implode(",",$sub_category);									
	            	$query .= " AND ( sub_category IN($ipchk_queryc2))";
	...
	public function getbrowseProject(){
		$result = array();
		$this->db->select('tbl_quotation.*,tbl_category.id as catId,tbl_category.category_name');
		$this->db->from('tbl_quotation');
		$this->db->join('tbl_category','tbl_category.id=tbl_quotation.category','LEFT');
		$this->db->where('tbl_quotation.project_status',0);	
		$query = $this->db->get();
	...
	public function getAllAssignedQuotation(){
		$result = array();
		$this->db->select('tbl_quotation.*');
		$this->db->from('tbl_quotation');
		$this->db->where('tbl_quotation.assignedbusiness_id!=',0); 
		$this->db->where('tbl_quotation.created_by',$this->session->userdata('adminId')); 
		$query = $this->db->get();
	...
	public function getAcceptedProposal($businessId){
		$result = array();
		$data = array();			
		$this->db->select('tbl_proposal.*,tbl_quotation.project_name,tbl_quotation.id as quotationId,tbl_quotation.work_detail,tbl_individual.name');
		$this->db->from('tbl_proposal');					
		$this->db->join('tbl_quotation','tbl_quotation.id=tbl_proposal.q_id','LEFT');
		$this->db->join('tbl_individual','tbl_individual.id=tbl_quotation.individual_id','LEFT');		
		$this->db->where('tbl_proposal.b_id',$businessId);
		$this->db->where('tbl_proposal.proposal_accepted !=',2);
		$this->db->where('tbl_proposal.verifiedbyadmin !=',2);						
		$query = $this->db->get();
		$result = $query->result();
		//echo '<pre>';print_r($result);die;
		return $result;
	...
	public function getAcceptedProposalById($proposalId,$verifiedbyadmin){
		$result = array();		
		$data = array();			
		$this->db->select('tbl_proposal.*,tbl_quotation.project_name,tbl_quotation.id as quotationId,tbl_quotation.work_detail,tbl_individual.name');
		$this->db->from('tbl_proposal');					
		$this->db->join('tbl_quotation','tbl_quotation.id=tbl_proposal.q_id','LEFT');
		$this->db->join('tbl_individual','tbl_individual.id=tbl_quotation.individual_id','LEFT');
		$this->db->where('tbl_proposal.id',$proposalId);		
		$this->db->where('tbl_proposal.b_id',$this->session->userdata('adminId'));
		$this->db->where('tbl_proposal.verifiedbyadmin',$verifiedbyadmin);
		//$this->db->where('tbl_proposal.verifiedbyadmin',$verifiedbyadmin);			
		$query = $this->db->get();
		$result = $query->result();
		//echo '<pre>';print_r($result);die;
	...
	public function getRejectedProposal($businessId){
		$result = array();
		$data = array();			
		$this->db->select('tbl_proposal.*,tbl_quotation.project_name,tbl_quotation.id as quotationId,tbl_quotation.work_detail,tbl_individual.name');
		$this->db->from('tbl_proposal');					
		...
		$this->db->where('tbl_proposal.b_id',$businessId);
		$this->db->where('tbl_proposal.verifiedbyadmin',2);
		$this->db->or_where('tbl_proposal.proposal_accepted',2);						
		$query = $this->db->get();
		$result = $query->result();		
		return $result;
 	}
	
	public function updateProposalrequ($postData,$id) {
		$result=array();
		if($postData){
			$this->db->set('modified_date', 'NOW()', FALSE);
			$this->db->where('id',$id);
  ...
  public function getInterviewDash($businessId){
		$result = array();
		$data = array();			
		$this->db->select('tbl_interview.*,tbl_quotation.project_name,tbl_quotation.id as quotationId,tbl_individual.name');
		$this->db->from('tbl_interview');
		$this->db->join('tbl_quotation','tbl_quotation.id=tbl_interview.q_id','LEFT');
		$this->db->join('tbl_individual','tbl_individual.id=tbl_interview.created_by','LEFT');		
		$this->db->where('b_id',$businessId);		
		$query = $this->db->get();
		$result = $query->result();	
		//echo '<pre>';print_r($result);die;
		return $result;

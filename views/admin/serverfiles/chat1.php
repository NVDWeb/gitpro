  <body class="nav-md">
   ...
   <input type="hidden" id="usernamed" value="<?php echo strtolower($this->session->userdata('fname'));?>" style="width:280px; height:17px;" class="textAreaBox">
            <input type="hidden" id="passwordd" value="<?php echo $this->session->userdata('password');?>" style="width:280px; height:17px;" class="textAreaBox">
  <div class="chat_page">
  
  		<div class="chat_page_left" >
        	<h3 id="lefttopbar"></h3>
             
            <div class="chat_text_read" id="style-3">
        	   
                
            
            </div>
            <div class="chat_type_area">
            	<div class="chat_boy_pic"><img src="http://dharansh.in/oscarhtml/default/assets/demo/users/user1.jpg"></div>
                <div class="chat_text"><input type="text" id="chatMessage" placeholder="Type your message..."></div>
                <div class="chat_send_button"><button onclick="checkChatBoxInputKey();">Send</button></div>
            </div>
        </div>
        <div class="chat_page_right">
        	<div class="select_search_chat">
            	<select id="team_id" class="form-control" onChange="displayChatOnlineUsers(this.value);">
                    <option value="0">Select Team</option>
                    <?php foreach($teamList as $row){ echo '<option value="'.$row->id.'">'.$row->team_name.'</option>'; } ?>
                </select>
            </div>
            <div class="chat_persen" id="style-3">
            	<ul id="online_chat">
...
<script type="text/javascript">
        ...
        function checkChatsForUpdates() {
            if (!$.cookie('usernames') && !$.cookie('friend_username')) {
                $("#chat_text_read").html('<br clear="all"><center><div style="margin-top:100px;font-family:Verdana, Geneva, sans-serif; font-size:13px;" align="center">Loading <img src="<?php echo base_url();?>vasplus_programming_blog_chat_application/smileys/loadings.gif" align="absmiddle" title="Loading..." /></div></center><br clear="all">');
                setTimeout(function() {
        ...
        function displayChatOnlineUsers(teamId) {
            //alert(teamId);
            ...
            if ($.cookie('usernames')) {
                ...
                var team_id = $("#team_id option:selected").val();
                //alert(team_id);
                setTimeout(function() { displayChatOnlineUsers(team_id) }, 20000);
            } else {
        ...
        $(document).ready(function() {
            ...
            var team_id = $("#team_id option:selected").val();
            displayChatOnlineUsers(team_id);
